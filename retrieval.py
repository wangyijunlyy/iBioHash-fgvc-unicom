...
parser.add_argument("--batch_size", default=256, type=int, help="The batch size to use for training and inference.")
parser.add_argument("--dataset", default="iBiohash", help="The dataset to load for training and evaluation.")
...
# 将时间格式化为字符串，例如：2024-04-11_15-30-00
...
def get_dataset(dataset_name: str, transform: Callable, transform_train=None) -> Dict:
    ...
    root = "/media/wsco/linux_gutai2/ibiohash"
    ...
    if dataset_name == "sop":
        ...
        return {"train": trainset, "test": testset, "metric": "rank1"}
    elif dataset_name == 'iBiohash':
        from dataset import IBioHash
        trainset = IBioHash(root, "train", transform_train)
        query = IBioHash(root, "query", transform)
        gallery = IBioHash(root, "gallery", transform)
        trainset.num_classes = trainset.nb_classes()
        return {"train": trainset, "query": query, "gallery": gallery, "metric": "rank1"}
    else:
        raise
...
def main():
    ...
    if args.eval:
        ...
        # print('>>>>>>>>>>>>>>>>>>>>>>>>\n',model_dict.keys())
        state_dict = torch.load('/home/wsco/wyj/unicom/output/2024-04-12_05-43-46/VITL14_model.pth')
        # print('>>>>>>>>>>>>>>>>>>>>>>>>\n',state_dict.keys())
        ...
        for epoch in range(0, args.epochs):
            ...
            for _, (img, local_labels) in pbar:
                img = img.cuda()
            ...
            if isinstance(score, float):
                if rank == 0:
                    print(f">>>>>>>>>>>>>>  eval result is {max_score}, epoch is {epoch+1}")
                if score > max_score: 
                    os.makedirs(os.path.join(args.output,time_str),exist_ok=True)
                    torch.save(model.state_dict(), os.path.join(args.output,time_str,'VITL14_model.pth'))
                    max_score = score
            model.train()
...
def evaluation(model: torch.nn.Module,
    ...
    if "index" in dataset_dict:
        ...
        q, q_label, q_image_ids = extract_feat(model, dataset_q, batch_size, num_workers)
        g, g_label,g_image_ids = extract_feat(model, dataset_g, batch_size, num_workers)
        import pickle
       # 保存 q_image_ids
        with open('q_image_ids.pkl', 'wb') as f:
            pickle.dump(q_image_ids, f)

        # 保存 g_image_ids
        with open('g_image_ids.pkl', 'wb') as f:
            pickle.dump(g_image_ids, f)
        faiss_index = faiss.IndexFlatL2(768)
        faiss_index.add(g)
        logging.debug("start searching")
        distances, predictions = faiss_index.search(q,100)
        ...
        with open('submit.csv', 'w') as f:
            ...
            for i, q_id in tqdm(enumerate(q_image_ids)):
                ...
                predicted_image_ids = [g_image_ids[idx] for idx in predicted_ids]  # 根据索引找到对应的图像ID
